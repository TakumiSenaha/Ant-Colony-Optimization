"""
先行研究（Previous Method）ACO Solverモジュール

【先行研究の概要】
エッジベースの学習機構を持つACO手法です。
各エッジが自身を通過したパスのボトルネック帯域の最小値w_ij^minと最大値w_ij^maxを
独自に学習・保持します。

【参考文献】
Senaha, T., Baba, T., & Asaka, T. (2025).
"A Distributed Approach for Maximum-Bandwidth-Path Discovery Using Ant Colony Optimization"
IEICE Communications Express, 14(11), 398-401.
DOI: 10.23919/comex.2025XBL0099
URL: https://ieeexplore.ieee.org/document/11157913

IEEE Xplore Article ID: 11157913

【提案手法との違い】
- 学習単位：エッジ（提案手法はノード）
- フェロモン更新：エッジの学習値（w_ij^min, w_ij^max）に基づく適応的付加
- フェロモン揮発：エッジの学習値に基づく適応的揮発
...
from ..modules.pheromone import (
    PreviousMethodPheromoneEvaporator,
    PreviousMethodPheromoneUpdater,
)
...
class PreviousMethodACOSolver:
    """
    先行研究（Previous Method）ACOソルバークラス

    エッジベースの学習機構を持つACOアルゴリズムです。
    特徴:
    - フェロモン更新: エッジの学習値（w_ij^min, w_ij^max）に基づく適応的付加
    - フェロモン揮発: エッジの学習値に基づく適応的揮発
    - ノード学習機能（BKB/BLD/BKH）は使用しない
    ...
    Attributes:
        ...
        pheromone_updater (PreviousMethodPheromoneUpdater): エッジ学習値ベースのフェロモン付加ロジック
        pheromone_evaporator (PreviousMethodPheromoneEvaporator): エッジ学習値ベースの揮発ロジック
        fluctuation_model (Optional[BandwidthFluctuationModel]): 帯域変動モデル（未使用ならNone）
        alpha (float): フェロモンの重み
        beta_bandwidth (float): 帯域ヒューリスティックの重み（通常1.0）
    ...
    def __init__(self, config: Dict, graph: RoutingGraph):
        ...
        # 先行研究用のフェロモン更新・揮発
        self.pheromone_updater = PreviousMethodPheromoneUpdater(config)
        self.pheromone_evaporator = PreviousMethodPheromoneEvaporator(config)
    ...
    def run(
        ...
        for generation in range(generations):
            # スタートノード切り替え処理（ConventionalACOSolverと同じロジック）
            if start_switching_enabled:
            ...
            results.append(
                {
                    ...
                    "optimal_solutions": current_gen_optimal_solutions,
                }
            )
    ...
    def _probabilistic_selection(self, ant: Ant, candidates: List[int]) -> int:
        """
        確率的選択（先行研究：フェロモンとヒューリスティック情報を使用）
