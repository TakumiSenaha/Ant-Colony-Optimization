...
【時間制限付きグローバル解】
- 動的環境への適応: グローバル解に世代情報を持たせ、1000世代を超えたら無効にする
- シンプルな実装: 履歴を保持せず、グローバル解とその世代のみを管理
- 動作: 1000世代を超えたら、次に来た解が新しいグローバル解になる
- 目的: 環境変化（帯域変動、ノード移動など）に追随しやすくする
- 注意: これによりACSは「完全なグローバル最適解」ではなく「最近の最良解」を参照する
"""
...
class ConventionalACOSolver:
    ...
    特徴:
    - 状態遷移: Pseudo-random-proportional rule（q0=0.9で最良エッジ選択、論文の標準値）
    - グローバル更新: 世代終了時に最良アリのみがフェロモンを更新（中央集権的）
      論文の式(4): τ_ij ← (1-ρ)τ_ij + ρΔτ_ij
    - ローカル更新: 解構築中に訪問エッジのフェロモンを即座に減少
      論文の式(3): τ_ij ← (1-ξ)τ_ij + ξτ_0（ξ=0.1、論文の標準値）
    - フェロモン更新: Δτ = Q * B_k（ボトルネック帯域に比例）
      注意: 論文ではΔτ_ij = 1/L_gb（TSPの経路長の逆数）だが、MBL問題に適用するため変更
    - ノード学習機能（BKB/BLD/BKH）は使用しない
    - 功績ボーナス/ペナルティは使用しない

    パラメータ（論文の標準値）:
    - α = 1.0（フェロモンの重み）
    - β = 2.0（論文の標準値、Dorigo & Gambardella 1997）
      * TSPではη_ij = 1/d_ij（距離の逆数）を使用
      * MBL問題ではη_ij = w_ij（帯域幅）を直接使用
      * 論文の標準値はβ = 2.0だが、設定ファイルで変更可能
      * 注意: beta_bandwidth = 0 の場合はヒューリスティックなし（従来手法1）
      *       beta_bandwidth = 1 の場合はヒューリスティックあり（従来手法2）
      *       beta_bandwidth = 2 の場合は論文の標準値（推奨）
    - q0 = 0.9（最良エッジを選択する確率）
    - ξ = 0.1（ローカル更新の強度）
    - τ_0 = 1/(n * L_nn)（TSPの場合）、本実装ではτ_0 = 100.0
      * TSPの式は問題の規模に応じて自動調整されるが、MBL問題では適用不可
      * 帯域幅の最大値（100Mbps）とmin_pheromone=100に合わせて設定
    ...
    Attributes:
        ...
        ttl (int): アリの最大ステップ数
        global_best_solution (Optional[Tuple[float, float, int]]): Global Best解（時間制限付き）
        global_best_ant (Optional[Ant]): Global Best解に対応するアリ
        global_best_generation (Optional[int]): グローバル解が記録された世代
        global_best_max_age (int): グローバル解の有効期限（世代数、デフォルト: 1000）
    ...
    def __init__(self, config: Dict, graph: RoutingGraph):
        ...
        # ACOパラメータ
        # 出典: Dorigo & Gambardella (1997)
        # IEEE Transactions on Evolutionary Computation, Vol. 1, No. 1, pp. 53-66
        # "Ant Colony System: A Cooperative Learning Approach to the Traveling Salesman Problem"
        self.alpha = config["aco"]["alpha"]  # 論文の標準値: α = 1.0
        # 論文の標準値: β = 2.0（TSPでもMBL問題でも同じ値を使用）
        self.beta_bandwidth = config["aco"]["beta_bandwidth"]
        self.beta_delay = config["aco"]["beta_delay"]
        # 論文の標準値: q0 = 0.9
        # Pseudo-random-proportional rule: 確率q0で最良エッジを確定的に選択
        self.q0 = config["aco"].get("q0", 0.9)
        ...
        # 初期フェロモン値τ_0
        # 論文の標準値（TSP）: τ_0 = 1/(n * L_nn)
        # 本実装（MBL問題）: "auto"の場合は最近傍ヒューリスティック解から自動計算
        #   τ_0 = B_nn / n（B_nnは最近傍解のボトルネック帯域、nはノード数）
        # 初期フェロモン値τ_0
        # 論文の標準値（TSP）: τ_0 = 1/(n * L_nn)
        # 本実装（MBL問題）: "auto"の場合は最近傍ヒューリスティック解から自動計算
        #   τ_0 = B_nn / n（B_nnは最近傍解のボトルネック帯域、nはノード数）
        # global_best_max_ageが十分大きい（>= 10000）場合は自動計算を復元（元の精度に戻す）
        initial_pheromone_config = config["aco"].get("initial_pheromone", 100.0)
        global_best_max_age = config["aco"].get("global_best_max_age", 10000)

        if initial_pheromone_config == "auto" or global_best_max_age >= 10000:
            # 自動計算を復元（元の精度に戻す）
            self.initial_pheromone = None  # 後で計算
            self._initial_pheromone_auto = True
        else:
            # 固定値を使用（精度を下げる）
            self.initial_pheromone = float(initial_pheromone_config)
            self._initial_pheromone_auto = False
        ...
        # Global Best（時間制限付きの最良解）
        # 1000世代を超えたら無効になる（動的環境への適応）
        ...
        # グローバル解の有効期限（世代数）
        # この世代数を超えたら、グローバル解を無効にする
        self.global_best_max_age = config["aco"].get("global_best_max_age", 10000)
    ...
    def _calculate_nearest_neighbor_heuristic(
        self, start_node: int, goal_node: int
    ) -> float:
        """
        最近傍ヒューリスティック解（貪欲法）のボトルネック帯域を計算

        TSPの最近傍法に対応するMBL問題の貪欲法:
        - 各ステップで最大帯域幅のリンクを選択
        - その経路のボトルネック帯域を返す
        ...
        while current != goal_node:
            ...
            # 未訪問の隣接ノードの中で最大帯域幅のリンクを選択
            for neighbor in self.graph.get_neighbors(current):
                if neighbor not in visited:
                    edge_attr = self.graph.get_edge_attributes(current, neighbor)
            ...
            if best_neighbor is None:
                ...
                try:
                    ...
                    # フォールバック: 帯域幅の平均値を使用
                    all_bandwidths = [
                        self.graph.get_edge_attributes(u, v)["bandwidth"]
                        for u, v in self.graph.graph.edges()
                    ]
                    return (
                        sum(all_bandwidths) / len(all_bandwidths)
                        if all_bandwidths
                        else 50.0
                    )

            # ボトルネックを更新
            min_bandwidth = min(min_bandwidth, best_bandwidth)
    ...
    def _calculate_initial_pheromone(self, start_node: int, goal_node: int) -> float:
        ...
        TSPの式 τ_0 = 1/(n * L_nn) に対応するMBL問題の式:
        τ_0 = B_nn / n
        ここで、B_nnは最近傍ヒューリスティック解のボトルネック帯域、nはノード数
        ...
        return max(tau_0, min_pheromone)
    ...
    def run(
        ...
        # 【初期フェロモン値の自動計算】"auto"またはglobal_best_max_age >= 10000の場合
        if self._initial_pheromone_auto and self.initial_pheromone is None:
            self.initial_pheromone = self._calculate_initial_pheromone(
                start_node, goal_node
            )
            print(
                f"初期フェロモン値τ_0を自動計算: {self.initial_pheromone:.2f} "
                f"(最近傍ヒューリスティック解のボトルネック帯域から算出)"
            )
        ...
        for generation in range(generations):
            ...
            # 論文の式(4): τ_ij ← (1-ρ)τ_ij + ρΔτ_ij
            # ここで、Δτ_ij = 1/L_gb（Global Best解の長さの逆数）
            # 注意: 本実装ではMBL問題に適用するため、Δτ_ij = Q * B_k（ボトルネック帯域）を使用
            ...
            if generation_ants:
                ...
                # 【時間制限付きグローバル解の更新】
                # 現在のグローバル解が有効期限を超えている場合は無効化
                if (
                    self.global_best_generation is not None
                    and generation - self.global_best_generation
                    >= self.global_best_max_age
                ):
                    # 1000世代を超えたら無効化（次に来た解が新しいグローバル解になる）
                    self.global_best_solution = None
                    self.global_best_ant = None
                    self.global_best_generation = None

                # グローバル解を更新（現在のグローバル解より良い場合、または無効な場合）
            ...
            # 出典: Dorigo & Gambardella (1997) - IEEE Transactions on Evolutionary Computation
            # 論文の式(4): τ_ij ← (1-ρ)τ_ij + ρΔτ_ij
            # 1. まず全エッジを揮発: τ_ij ← (1-ρ)τ_ij（ρは揮発率、論文では0.1が標準）
            # 2. その後、Global Bestの経路にフェロモンを付加: τ_ij ← τ_ij + ρ/L_gb
            #    注意: 論文ではΔτ_ij = 1/L_gb（TSPの経路長の逆数）だが、
            #          本実装ではMBL問題に適用するため、Δτ_ij = Q * B_k（ボトルネック帯域）を使用
    ...
    def _select_next_node(self, ant: Ant) -> Optional[int]:
        ...
        論文の式に基づく状態遷移ルール:
        - 確率q0（論文の標準値: 0.9）で最良エッジを確定的に選択（exploitation）
          最良エッジ: τ_ij^α * η_ij^βが最大のエッジ
        - 確率(1-q0)で確率的選択（biased exploration）
          確率分布: p_ij^k = [τ_ij]^α · [η_ij]^β / Σ_l [τ_il]^α · [η_il]^β
    ...
    def _select_best_edge(self, ant: Ant, candidates: List[int]) -> int:
        ...
        【ACS方式のExploitation】
        論文のPseudo-random-proportional ruleに基づく:
        - 確率q0（論文の標準値: 0.9）で最良エッジを確定的に選択
        - 最良エッジ: argmax_j {τ_ij^α * η_ij^β}
        - これにより、探索（exploration）と活用（exploitation）のバランスを制御する
    ...
    def _apply_local_update(self, u: int, v: int) -> None:
        ...
        論文の式（式(3)）: τ_ij ← (1-ξ)τ_ij + ξτ_0
        - ξ（論文の標準値: 0.1）: ローカル更新の強度
        - τ_0: 初期フェロモン値（論文では1/(n * L_nn)、本実装では100.0）

        【目的】
        同じ世代内で複数のアリが同一経路に収束することを防ぎ、探索の多様性を維持する。
        論文では「Local Updating Rule」として、解構築中に訪問したエッジのフェロモンを
        即座に減少させることで、探索の多様性を確保している。
